...
// BookStack page edit button
const pageBtn = document.createElement("button");
pageBtn.id = "floating-edit-btn";
pageBtn.classList.add("floating-btn");
pageBtn.innerHTML = `
...
`;
pageBtn.onclick = () => { window.location.href = window.location.href + "/edit"; };
...
const h1 = document.querySelector("#bkmrk-page-title");
const pageTitle = h1 ? h1.innerText.trim() : "Untitled";

const titleBar = document.createElement("div");
titleBar.className = "sheet-title-bar";
titleBar.innerText = pageTitle;

// Insert title bar before iframe
...
updateTitleBar(); // run initially

// Patch toggle button handler to also update titleBar
const originalToggle = sheetBtn.onclick;
sheetBtn.onclick = () => {
  originalToggle();
  updateTitleBar();
};
